...
const UserDetailsModal = ({
  ...
  const doPrint = async () => {
    try {
      ...
      printWindow.document.write(
        `<!doctype html><html><head><meta charset="utf-8" />${headHtml}<style>body{display:flex;align-items:center;justify-content:center;min-height:100vh;background:#f3f4f6;padding:20px}.card-container{box-shadow:none!important}</style></head><body>${cardHtml}</body></html>`,
      );
  ...
  const applyCrop = async () => {
    ...
    try {
      ...
      const file = new File([blob], selectedFile.name || 'photo.png', {
        type: 'image/png',
      });
  ...
  const handleSaveChanges = async () => {
    ...
    try {
      ...
      const res = await client(`/auth/users/${user._id}`, {
        method: 'PUT',
        body,
      });
            ...
            <div className='fixed inset-0 z-60 flex items-center justify-center bg-black bg-opacity-60'>
              <div className='bg-white rounded-lg max-w-4xl w-full p-4'>
                <div className='flex justify-between items-center mb-3'>
                  ...
                  <button onClick={cancelCrop} className='text-gray-500'>
                    ✕
                  </button>
                </div>
                <div className='flex flex-col md:flex-row gap-4'>
                  <div
                    className='flex-1 bg-gray-50 p-2 relative'
                    style={{ height: 420 }}
                  >
                    {tempPreviewUrl ? (
            ...
            <div>
              ...
              <div className='space-y-2 text-sm'>
                ...
                <div>
                  ...
                  <span className='ml-2 text-gray-900'>
                    {user.memberNumber ? `#${user.memberNumber}` : 'N/A'}
                  </span>
                </div>
                <div>
              ...
              <div className='flex flex-wrap gap-4'>
                {user.aadharFront ? (
                  <div className='w-44'>
                    ...
                    <a
                      href={getImageUrl(user.aadharFront)}
                      target='_blank'
                      rel='noreferrer'
                      className='block'
                    >
                      <img
                        src={getImageUrl(user.aadharFront)}
                        alt={`${user.username}-aadhar-front`}
                ...
                {user.aadharBack ? (
                  <div className='w-44'>
                    ...
                    <a
                      href={getImageUrl(user.aadharBack)}
                      target='_blank'
                      rel='noreferrer'
                      className='block'
                    >
                      <img
                        src={getImageUrl(user.aadharBack)}
                        alt={`${user.username}-aadhar-back`}
                        className='w-44 h-28 object-cover rounded border'
                      />
                    </a>
                    <div className='text-xs text-gray-500 mt-1'>
                ...
                <div className='grid md:grid-cols-2 gap-3'>
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setForm((s) => ({ ...s, contactNo: e.target.value }))
                      }
                      className='w-full border px-3 py-2 rounded'
                    />
                  </div>
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setForm((s) => ({ ...s, fatherName: e.target.value }))
                      }
                      className='w-full border px-3 py-2 rounded'
                    />
                  </div>
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setForm((s) => ({ ...s, motherName: e.target.value }))
                      }
                      className='w-full border px-3 py-2 rounded'
                    />
                  </div>
                ...
                (user.isTeamLeader ? (
                  <button
                    onClick={async () => {
                      if (
                        !confirm(
                          `Revoke team leader status from ${user.username}?`,
                        )
                      )
                        return;
                      setLoading(true);
                  ...
                  <button
                    onClick={async () => {
                      if (
                        !confirm(
                          `Make ${user.username} a team leader? This will generate a referral code for them.`,
                        )
                      )
                        return;
                      setLoading(true);
                ...
                (user.role !== 'admin' ? (
                  <button
                    onClick={async () => {
                      if (
                        !confirm(
                          `Make ${user.username} an admin? This will grant full admin privileges.`,
                        )
                      )
                        return;
                      setLoading(true);
