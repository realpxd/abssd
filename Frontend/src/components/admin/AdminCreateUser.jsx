...
const AdminCreateUser = ({ onCreated, onCancel }) => {
    ...
    if (!emailError && formData.email) {
      ...
      try {
        const emailResponse = await client(
          ...
          { method: 'POST', body: { email: formData.email } },
        );
    ...
    if (!contactError && formData.contactNo) {
      try {
        const contactResponse = await client(
          ...
          { method: 'POST', body: { contactNo: formData.contactNo } },
        );
        if (contactResponse.exists) {
          setContactError('Contact already registered');
          setValidating(false);
          return false;
        }
      } catch (e) {
        setError('Error checking contact');
        setValidating(false);
        return false;
      }
    }
  ...
  const handleRegister = async () => {
    ...
    try {
      ...
      const result = await client(api.endpoints.auth + '/register', {
        method: 'POST',
        body: payload,
      });
    ...
    try {
      ...
      if (newUser && newUser._id) {
        try {
          ...
          await client(
            api.endpoints.auth + `/users/${newUser._id}/membership`,
            {
              method: 'PUT',
      ...
      <div className='bg-gradient-to-r from-orange-500 to-orange-600 rounded-lg shadow-lg p-6 mb-4 text-white'>
        ...
        <div className='grid md:grid-cols-2 gap-4'>
          {Object.entries(MEMBERSHIP_PLANS).map(([key, plan]) => (
            <div
              key={key}
              className={`p-4 rounded border ${selectedPlan === key ? 'border-white' : 'border-transparent'}`}
            ...
            <div className='flex items-center space-x-4 mb-4'>
              <label className='flex items-center space-x-2'>
                ...
                <span className='text-sm'>
                  Skip payment (create user without collecting payment)
                </span>
              </label>
            </div>
