...
// ----------------- Hook: fit text into max lines using binary search -----------------
const useFitText = (ref, deps = [], options = {}) => {
  const { minFont = 11, maxFont = 18, maxLines = 2 } = options;
  ...
  useEffect(() => {
    ...
    const compute = () => {
      ...
      const defaultFontSize = parseFloat(comp.fontSize) || maxFont;
      const lineHeightPx = parseFloat(comp.lineHeight) || defaultFontSize * 1.2;

      let lo = minFont;
      let hi = maxFont;
      let best = minFont;

      // binary search for largest font that fits within maxLines
      while (lo <= hi) {
    ...
    // deps should include anything that changes text or container size
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [ref, ...deps, minFont, maxFont, maxLines]);

  return fontSize;
};

// Small presentational subcomponent for the name area. Kept pure and memoized.
const NameArea = ({ rawName, formatName, headlineColor }) => {
  const expand = rawName.length > 12;
  const display = expand ? rawName : formatName(rawName);
  const nameRef = useRef(null);
  const fontSize = useFitText(nameRef, [display], {
    minFont: 11,
    maxFont: 17,
    maxLines: 2,
  });
  ...
  return (
    <>
      <div className='flex gap-1 items-start'>
        ...
        <div
          ref={nameRef}
          style={{ color: headlineColor, fontSize: `${fontSize}px` }}
          className={`text-[16px] font-extrabold ${expand ? 'max-w-[260px]' : 'max-w-[130px]'} break-words underline`}
        >
          {display}
        </div>
      </div>

      {/* second name slot intentionally left empty; used as extension area when primary name expands */}
      <div className='mb-1 flex gap-1 items-start'>
        <div className='text-gray-700 text-[15px] font-semibold'> </div>
        <div className='text-[16px] font-extrabold max-w-[130px] break-words underline'>
          {/* intentionally blank */}
        </div>
      </div>
    </>
...
const IDCard = forwardRef(
  ({ user, photoPreview, watermarkText, design }, ref) => {
    if (!user) return null;

    const photoSrc =
      photoPreview || (user.photo ? getImageUrl(user.photo) : null);
    ...
    const variant =
      design || (membershipType === 'annual' ? 'annual' : 'ordinary');
    ...
    // --- Utilities moved outside component body where possible ---
    // (kept here to preserve file-localization but lifted to top-level below)

    // useMemo-friendly derived values
    ...
    return (
      <div
        ref={ref}
        className='relative w-full rounded-xl shadow-md overflow-hidden flex flex-col card-container'
        style={{
          width: '298.58px',
          height: '396.8px',
          border: '1px solid rgba(35,48,63,0.06)',
      ...
      >
        ...
        <div className='absolute top-0 right-0 w-20 h-20 rotate-90 pointer-events-none'>
          <FloralCorner />
        </div>
        <div className='absolute bottom-0 left-0 w-20 h-20 -rotate-90 pointer-events-none'>
          <FloralCorner />
        </div>
        ...
        {/* Header */}
        <div
          className='h-fit px-4 py-1 flex items-center gap-2'
        ...
        >
          ...
          <div
            className='flex gap-2 px-3 py-3 relative flex-1'
            style={
              {
                // background: bodyBg
          ...
          >
              ...
              >
                <defs>
                  <linearGradient id='g1' x1='0' x2='1'>
                    <stop
                      offset='0%'
                      stopColor={variant === 'annual' ? '#fff7ed' : '#fffaf0'}
                    />
                ...
                <g
                  ...
                  fill='none'
                  stroke={variant === 'annual' ? '#ffd8a8' : '#ffedd5'}
                  strokeWidth='1'
                >
            ...
            >
              ...
              <div
                style={{
                  background: variant === 'annual' ? '#fff7e2' : '#fffaf0',
                }}
                className='text-[10px] text-center font-bold px-2 py-1 rounded-full text-orange-700'
              >
            ...
            <div
              className='flex-1 flex flex-col gap-2 text-[14px]'
              style={{ paddingTop: 6 }}
            >
              {/* Name area: primary name may expand into the secondary area when long */}
              <NameArea
                rawName={rawName}
          ...
          <div className='flex flex-col items-center justify-center align-middle w-full'>
            <div className='mb-2 flex gap-1 w-full items-start align-middle justify-center'>
              ...
              <div
                className='text-gray-800 max-w-[200px] break-words'
                style={emailStyleFor(user.email)}
              >
                {user.email || 'N/A'}
              </div>
            </div>
          ...
          >
            <div className='w-1/2 pr-2'>
              <div
                style={{ color: headlineColor, fontWeight: 700, fontSize: 18 }}
              >
                राष्ट्रीय अध्यक्ष
              </div>
              {!watermarkText && (
              ...
              <div
                className={`${watermarkText ? 'h-7' : 'h-2'} border-b border-dashed border-gray-300 my-0 mr-8`}
              ></div>
            </div>
            <div className='w-1/2 flex items-center justify-end pr-1 mt-2'>
              ...
              >
                <div
                  ...
                  className='bg-white flex items-center justify-center rounded-sm overflow-hidden'
                >
                  <img
                ...
                <div className='text-gray-800 text-[10px] font-bold leading-tight'>
                  https://abssd.in
                </div>
              </div>
            </div>
