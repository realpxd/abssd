...
const Register = () => {
    ...
    const load = async () => {
      try {
        const resp = await client(
          api.endpoints.geo + `/cities/${encodeURIComponent(state)}`,
        );
        if (resp && resp.data && Array.isArray(resp.data) && mounted) {
          ...
          if (resp.data.length && !resp.data.includes(formData.address.city)) {
            setFormData((s) => ({
              ...s,
              address: { ...s.address, city: resp.data[0] },
            }));
          }
        } else {
          setCityOptions([]);
  ...
  const handleEmailBlur = async () => {
    ...
    try {
      ...
      if (response.exists) {
        setEmailError(
          'यह ईमेल पहले से पंजीकृत है। कृपया लॉगिन करें / This email is already registered. Please login.',
        );
      }
    } catch (error) {
  ...
  const handleContactBlur = async () => {
    ...
    try {
      ...
      if (response.exists) {
        setContactError(
          'यह संपर्क नंबर पहले से पंजीकृत है / This contact number is already registered',
        );
      }
    } catch (error) {
      // Silently fail - don't show error for validation check
  ...
  const validateStep1 = async () => {
    ...
    if (!contactError && phoneValidation.sanitized) {
      try {
        ...
        setError(
          'संपर्क नंबर जांचने में त्रुटि / Error checking contact number',
        );
        setValidating(false);
        return false;
      }
  ...
  const handlePayment = async () => {
    ...
    try {
      ...
      const orderResponse = await client(
        api.endpoints.payment + '/create-order',
        {
          method: 'POST',
          body: {
      ...
      const options = {
        ...
        handler: async (response) => {
          try {
            ...
            navigate(
              `/payment-status?orderId=${response?.razorpay_order_id || orderResponse.data.orderId}`,
            );
          }
        },
        modal: {
      ...
      setError(
        'initiation error: ' + (err.message || 'Error initiating payment'),
      );
      setPaymentLoading(false);
    }
  };
  ...
  const handleRegister = async () => {
    ...
    try {
      ...
      if (formData.referralCode)
        payload.append('referralCode', formData.referralCode);
      ...
      payload.append(
        'aadharConfirmed',
        formData.aadharConfirmed ? 'true' : 'false',
      );
  ...
  const validateAadhaarStep = () => {
    if (!formData.aadharFront || !formData.aadharBack) {
      setError(
        'कृपया दोनों आधार की छवियाँ अपलोड करें / Please upload both front and back images of your Aadhaar',
      );
      return false;
    }
        ...
        <div className='max-w-4xl mx-auto'>
          ...
          <div className='bg-white rounded-lg shadow-lg p-8'>
            ...
            {step === 1 && (
              <div className='space-y-6'>
                ...
                <div className='grid md:grid-cols-2 gap-6'>
                  ...
                  <div>
                    <label className='block text-sm font-medium text-gray-700 mb-2'>
                      Referral Code (To select Team leader)
                    </label>
                    <input
                      type='text'
                      name='referralCode'
                    ...
                    <div className='grid md:grid-cols-3 gap-2'>
                      ...
                      >
                        ...
                        {cityOptions.length > 0 ? (
                          ...
                          <option value={formData.address.city}>
                            {formData.address.city || 'No cities available'}
                          </option>
                        )}
                      </select>
            ...
            {step === 3 && (
              <div className='space-y-6'>
                <h3 className='text-xl font-semibold mb-4'>
                  आधार अपलोड करें / Upload Aadhaar
                </h3>
                <div className='grid md:grid-cols-2 gap-6'>
                  <div>
                    <label className='block text-sm font-medium text-gray-700 mb-2'>
                      Aadhaar Front (अपलोड करें)
                    </label>
                    <input
                      type='file'
                      name='aadharFront'
                    ...
                    {formData.aadharFront && (
                      <img
                        ...
                        alt='Aadhaar Front'
                        className='mt-3 w-48 h-auto rounded shadow'
                      />
                    )}
                  </div>
                    ...
                    <label className='inline-flex items-center'>
                      <input
                        ...
                        onChange={(e) =>
                          setFormData({
                            ...formData,
                            aadharConfirmed: e.target.checked,
                          })
                        }
                        className='mr-2'
                      />
                      मैं पुष्टि करता हूँ कि ऊपर अपलोड की गई आधार छवियाँ मेरी
                      हैं / I confirm the uploaded Aadhaar images are mine
                    </label>
                  </div>
                </div>
            ...
            {step === 4 && (
              <div className='space-y-6'>
                <h3 className='text-xl font-semibold mb-4'>
                  समीक्षा और भुगतान / Review & Payment
                </h3>
                <div className='bg-gray-50 p-6 rounded-lg mb-6'>
                  <h4 className='font-semibold mb-4'>
                    चयनित योजना / Selected Plan
                  </h4>
                  <div className='mb-4'>
                  ...
                  <div className='text-sm text-gray-600'>
                    <p>
                      कृपया अपनी जानकारी की समीक्षा करें और भुगतान करने के लिए
                      आगे बढ़ें।
                    </p>
                    <p>
