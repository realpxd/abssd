...
const reconcileOnce = async () => {
  ...
  try {
    ...
    const attempts = await PaymentAttempt.find({ status: { $in: ['created','pending'] }, createdAt: { $lte: threshold } }).limit(50)
        ...
        if (captured) {
          ...
          if (email) {
            ...
            if (user && user.membershipStatus !== 'active') {
              ...
              const membership = (a.membershipType) || user.membershipType || 'ordinary'
              ...
              await User.findByIdAndUpdate(user._id, {
                ...
                membershipAmount: (captured.amount / 100) || user.membershipAmount,
              ...
              try {
                ...
                logger.warn('Failed to send activation email during reconcile:', mailErr)
              }
            }
          }
        }
