...
exports.getCitiesByState = async (req, res) => {
  try {
    ...
    return res
      .status(500)
      .json({ success: false, message: 'Failed to fetch cities for state' });
  }
};
...
exports.getByPincode = async (req, res) => {
  try {
    ...
    if (!Array.isArray(data) || data.length === 0) {
      return res
        .status(500)
        .json({ success: false, message: 'Failed to lookup pincode' });
    }
    ...
    res
      ...
      .json({
        success: false,
        message: error.message || 'Error fetching pincode info',
      });
  }
};
