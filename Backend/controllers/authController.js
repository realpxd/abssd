...
exports.register = async (req, res) => {
  try {
    ...
    const aadharConfirmed =
      req.body.aadharConfirmed === 'true' || req.body.aadharConfirmed === true;
    ...
    const aadharFrontPath = aadharFrontFile
      ? `/uploads/${aadharFrontFile.filename}`
      : undefined;
    const aadharBackPath = aadharBackFile
      ? `/uploads/${aadharBackFile.filename}`
      : undefined;
    ...
    try {
      let defaultPos = await Position.findOne({
        name: { $regex: /^member$/i },
      });
      if (!defaultPos) {
        defaultPos = await Position.create({
          name: 'Member',
          slug: 'member',
          description: 'Default member position',
        });
      }
    ...
    if (error && (error.code === 11000 || error.name === 'MongoServerError')) {
      ...
      return res
        .status(409)
        .json({ success: false, message: `${dupField} already exists` });
    }
    res.status(500).json({
      success: false,
...
exports.forgotPassword = async (req, res) => {
  try {
    ...
    res.status(200).json({
      success: true,
      message:
        'Password reset token generated and emailed if the address exists',
    });
  } catch (error) {
    res.status(500).json({
      success: false,
...
exports.testEmail = async (req, res) => {
  try {
    ...
    await mail.sendMail({
      to,
      subject: subject || 'Test email from ABSSD',
      html: `<p>${(message || 'This is a test email from ABSSD').replace(/\n/g, '<br/>')}</p>`,
    });
    res
      .status(200)
...
exports.updateTeamLeader = async (req, res) => {
  try {
    ...
    if (typeof isTeamLeader === 'undefined') {
      return res
        .status(400)
        .json({ success: false, message: 'isTeamLeader flag is required' });
    }
    ...
    try {
      const subject = user.isTeamLeader
        ? 'You are now a Team Leader'
        : 'Your Team Leader status has been revoked';
...
exports.updateUserPosition = async (req, res) => {
  try {
    ...
    try {
      ...
      const subject = posName
        ? `You have been assigned the position: ${posName}`
        : `Your position has been cleared`;
      const html = posName
...
exports.updateMemberNumber = async (req, res) => {
  try {
    ...
    if (memberNumber === undefined || memberNumber === null) {
      return res
        .status(400)
        .json({ success: false, message: 'memberNumber is required' });
    }
  ...
  try {
    ...
    try {
      ...
      await mailer.sendMail({
        to: user.email,
        subject: subject || 'Notification from ABSSD Trust',
        html,
      });
    } catch (mailErr) {
    ...
    if (user.photo && user.photo.startsWith('/uploads/')) {
      ...
      if (fs.existsSync(filePath)) {
        try {
          ...
          const logger = require('../utils/logger');
          logger.warn(
            'Failed to delete user photo during user deletion:',
            unlinkErr,
          );
        }
      }
